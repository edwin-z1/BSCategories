...
@implementation UIDevice (BSCategory)
...
- (BOOL)bs_isJailBroken {
    if ([self bs_isSimulator]) return NO; // Dont't check simulator
    
    // iOS9 URL Scheme query changed ...
    if (self.systemVersion.doubleValue >= 9.0) {
    ...
    NSString *path = [NSString stringWithFormat:@"/private/%@", [NSString bs_UUIDString]];
...
- (nullable NSString *)ipAddressWithIfaName:(NSString *)name {
...
- (int64_t)bs_diskSpaceUsed {
    int64_t total = self.bs_diskSpace;
    int64_t free = self.bs_diskSpaceFree;
...
- (float)bs_cpuUsage {
    float cpu = 0;
    NSArray *cpus = self.bs_cpuProcessorsUsage;
