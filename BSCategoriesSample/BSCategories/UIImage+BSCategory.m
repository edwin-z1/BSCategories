...
typedef NS_ENUM(NSUInteger, BSImageFlipDirection) {
    BSImageFlipDirectionHorizontal,
    BSImageFlipDirectionVertical
};
...
+ (instancetype)bs_imageWithEmoji:(NSString *)emoji width:(CGFloat)width {
    ...
    CTFontRef font = CTFontCreateWithName(CFSTR("AppleColorEmoji"), width * scale, NULL);
    NSAssert(font, @"can not create CTFontRef");
    ...
    CGContextRef ctx = CGBitmapContextCreate(NULL, width * scale, width * scale, 8, 0, colorSpace, kCGBitmapByteOrderDefault | kCGImageAlphaPremultipliedFirst);
...
+ (nullable instancetype)bs_imageWithPDFData:(NSData *)data {
    return [self imageWithPDF:data size:CGSizeZero];
}

+ (nullable instancetype)bs_imageWithPDFData:(NSData *)data size:(CGSize)size {
    return [self imageWithPDF:data size:size];
}
...
+ (nullable instancetype)bs_imageWithPDFPath:(NSString *)path size:(CGSize)size {
    return [self imageWithPDF:path size:size];
}

+ (nullable instancetype)imageWithPDF:(id)dataOrPath size:(CGSize)size {
    ...
    CGSize pdfSize = (size.width + size.height == 0) ? pdfRect.size : size;
    CGFloat scale = [UIScreen mainScreen].scale;
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
...
+ (instancetype)bs_imageWithGIFData:(NSData *)data {
    ...
    if (count <= 1) {
        ...
        float (^caculateFrameDuration)(NSUInteger, CGImageSourceRef) = ^(NSUInteger index, CGImageSourceRef source){
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            duration += caculateFrameDuration(i, source);
...
- (instancetype)bs_imageByApplyingLightEffect {
    ...
    return [self bs_imageByApplyingBlurWithRadius:30 tintColor:tintColor saturationDeltaFactor:1.8 maskImage:nil];
}

- (instancetype)bs_imageByApplyingExtraLightEffect {
    UIColor *tintColor = [UIColor colorWithWhite:0.97 alpha:0.82];
    return [self bs_imageByApplyingBlurWithRadius:20 tintColor:tintColor saturationDeltaFactor:1.8 maskImage:nil];
}
...
- (instancetype)bs_imageByApplyingTintEffectWithColor:(UIColor *)tintColor {
    ...
    return [self bs_imageByApplyingBlurWithRadius:10 tintColor:effectColor saturationDeltaFactor:-1.0 maskImage:nil];
}
...
- (instancetype)bs_imageByApplyingBlurWithRadius:(CGFloat)blurRadius tintColor:(UIColor *)tintColor saturationDeltaFactor:(CGFloat)saturationDeltaFactor maskImage:(nullable UIImage *)maskImage {
    ...
    NSAssert(self.size.width >= 1 && self.size.height >= 1, @"*** error: invalid size: (%.2f x %.2f). Both dimensions must be >= 1: %@", self.size.width, self.size.height, self);
    NSAssert(self.CGImage, @"*** error: image must be backed by a CGImage: %@", self);
    if (maskImage) {
        NSAssert(maskImage.CGImage, @"*** error: maskImage must be backed by a CGImage: %@", maskImage);
    }
...
- (instancetype)bs_imageByCornerRadius:(CGFloat)radius {
    return [self bs_imageByCornerRadius:radius borderWidth:0 borderColor:nil];
}
...
- (instancetype)bs_imageByCornerRadius:(CGFloat)radius borderWidth:(CGFloat)borderWidth borderColor:(nullable UIColor *)borderColor {
    return [self bs_imageByCornerRadius:radius corners:UIRectCornerAllCorners borderWidth:borderWidth borderColor:borderColor borderLineJoin:kCGLineJoinMiter];
}
...
- (nullable instancetype)bs_imageByRotateDegree:(CGFloat)degree {
    CGFloat radian = BSDegreeToRadian(degree);
    size_t width = (size_t)CGImageGetWidth(self.CGImage);
    size_t height = (size_t)CGImageGetHeight(self.CGImage);
    CGRect newRect = CGRectApplyAffineTransform(CGRectMake(0., 0., width, height),CGAffineTransformMakeRotation(radian));
...
- (nullable instancetype)bs_imageByRotateLeft90 {
    return [self bs_imageByRotateDegree:90];
}
...
- (nullable instancetype)bs_imageByRotate180 {
    return [self bs_imageByRotateDegree:180];
}

- (nullable instancetype)bs_imageByFlipVertical {
    return [self flip:(BSImageFlipDirectionVertical)];
}

- (nullable instancetype)bs_imageByFlipHorizontal {
    return [self flip:(BSImageFlipDirectionHorizontal)];
}

- (nullable instancetype)flip:(BSImageFlipDirection)flipDirection {
    ...
    if (flipDirection == BSImageFlipDirectionVertical) {
        vImageVerticalReflect_ARGB8888(&src, &dest, kvImageBackgroundColorFill);
    }
    if (flipDirection == BSImageFlipDirectionHorizontal) {
